/*====================================================================================================================================
 ...
 PARAMETERS:		None.
 USAGE:				EXEC [ELT].[usp_Load_Treasury_MonetaryAsset] 20180320
...
======================================================================================================================================
*/
CREATE   PROCEDURE [ELT].[usp_Load_Treasury_MonetaryAsset]
@FileVersionId INT 
AS
BEGIN 
...
IF OBJECT_ID(N'[Treasury].[MonetaryAsset]', N'U') IS NULL
...
DECLARE @SummaryOfChanges TABLE(Change VARCHAR(20)); 
	...
	MERGE Treasury.MonetaryAsset AS Target
	USING ( SELECT [Cash and other monetary assets (In billions of dollars)]
            ,[Government Type]
            ,[2009]
            ,[2010]
            ,[2011]
            ...
            ,[2016]
            ,[FileVersionId]
            ,HASHBYTES ('SHA2_256', ISNULL(RTRIM([Cash and other monetary assets (In billions of dollars)]),'')+ISNULL(RTRIM([Government Type]),'')+ISNULL(RTRIM([2009]),'')+ISNULL(RTRIM([2010]),'')+ISNULL(RTRIM([2011]),'')+ISNULL(RTRIM([2012]),'')+ISNULL(RTRIM([2013]),'')+ISNULL(RTRIM([2014]),'')+ISNULL(RTRIM([2015]),'')+ISNULL(RTRIM([2016]),'')) AS RowHash
          FROM [Staging].[Note 2]
		   ) AS Source
	...
	WHEN MATCHED THEN UPDATE SET 
	    Target.[Cash and other monetary assets (In billions of dollars)] = Source.[Cash and other monetary assets (In billions of dollars)]
	...
	WHEN NOT MATCHED BY TARGET THEN INSERT
		([Cash and other monetary assets (In billions of dollars)]
    ,[GovernmentType]
		,[2009]
		,[2010]
		,[2011]
    ...
    ,[2016]
		...
		VALUES (
		 Source.[Cash and other monetary assets (In billions of dollars)]
