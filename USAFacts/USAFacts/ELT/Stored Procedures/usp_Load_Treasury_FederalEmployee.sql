/*====================================================================================================================================
 ...
 PARAMETERS:		None.
 USAGE:				EXEC [ELT].[usp_Load_Treasury_FederalEmployee] 20180320
...
======================================================================================================================================
*/
CREATE   PROCEDURE [ELT].[usp_Load_Treasury_FederalEmployee]
@FileVersionId INT 
AS
BEGIN 
...
IF OBJECT_ID(N'[Treasury].[FederalEmployee]', N'U') IS NULL
...
DECLARE @SummaryOfChanges TABLE(Change VARCHAR(20)); 
	...
	MERGE Treasury.FederalEmployee AS Target
	USING ( SELECT [Federal employee and veteran benefits payable (In billions of do]
            ,[Category]
            ,[Government Type]
            ,[2009]
            ,[2010]
            ...
            ,[2016]
            ,[FileVersionId]
            ,HASHBYTES ('SHA2_256', ISNULL(RTRIM([Federal employee and veteran benefits payable (In billions of do]),'')+ISNULL(RTRIM([Category]),'')+ISNULL(RTRIM([Government Type]),'')+ISNULL(RTRIM([2009]),'')+ISNULL(RTRIM([2010]),'')+ISNULL(RTRIM([2011]),'')+ISNULL(RTRIM([2012]),'')+ISNULL(RTRIM([2013]),'')+ISNULL(RTRIM([2014]),'')+ISNULL(RTRIM([2015]),'')+ISNULL(RTRIM([2016]),'')) AS RowHash
          FROM [Staging].[Note 12]
		   ) AS Source
		ON (Target.FileVersionId = Source.FileVersionId
				AND Target.RowHash = Source.RowHash)
		
	WHEN MATCHED THEN UPDATE SET 
	    Target.[Federal employee and veteran benefits payable (In billions of do] = Source.[Federal employee and veteran benefits payable (In billions of do]
	...
	WHEN NOT MATCHED BY TARGET THEN INSERT
		([Federal employee and veteran benefits payable (In billions of do]
		,[Category]
    ,[GovernmentType]
		,[2009]
		,[2010]
    ...
    ,[2016]
		...
		VALUES (
		 Source.[Federal employee and veteran benefits payable (In billions of do]
    ,Source.Category
